    ...
    try {
      if (op === 1) {
        if ((nombre !== '') && (numero !== '') && (modelo !== '') && (local !== '') && (numero.length === 10)) {
          ...
          }).then(alert('Elementos ingresados correctamente'), setData([...data, res.data]), setNombre(''), setNumero(''), setModelo(''), setLocal('')).catch(error => { alert('Error 403, no tiene permisos'); })
